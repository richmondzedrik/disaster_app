...
function Test-SensitiveFiles {
    ...
    foreach ($pattern in $patterns) {
        ...
        if ($files) {
            Write-Host "WARNING: Found potentially sensitive file pattern: $pattern" -ForegroundColor Red
            $sensitiveFound = $true
        }
    }
    ...
    if ($stagedFiles) {
        ...
        foreach ($file in $stagedFiles) {
            if ($file -match '\.(sql|env|key|pem|crt)$' -or $file -match '(secret|password|credential)') {
                Write-Host "   WARNING: $file (potentially sensitive)" -ForegroundColor Red
            } else {
...
function Invoke-InteractiveCommit {
    ...
    if ($addAll -eq 'y' -or $addAll -eq 'Y') {
        ...
        Write-Host "Use git add to stage specific files" -ForegroundColor Yellow
        return
    }
...
function Invoke-DeployPrep {
    ...
    if ($status) {
        Write-Host "Working directory is not clean" -ForegroundColor Yellow
        git status --short
    ...
    Write-Host "   1. Run: .\deploy.sh" -ForegroundColor White
